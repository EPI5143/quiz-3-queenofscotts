...
##################
carsplot <-ggplot(data = mpg) + geom_point(mapping = aes(x = displ, y = hwy))
##################
# Question 2. modify and run the ggplot code to make each class of vehicle a different colour
carsplot + geom_point(mapping= aes(x = displ, y = hwy,colour=class))
...
carsplot + geom_point(mapping= aes(x = displ, y = hwy,colour=class, shape=drv))
...
Finalplot <- carsplot + geom_point(mapping= aes(x = displ, y = hwy,colour=class, shape=drv, size=cyl))

# Question 5. Modify the code to add a suitable title of your choice to your plot
Finalplot +labs(title= 'Car info', x = 'engine size', y = 'highway fuel efficiency')
